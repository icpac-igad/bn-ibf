...
<p align="center">
  <img src="docs/ibf-bn.drawio.png" width="100%" />
</p>

Bayesian Network(BN) and Decision making, Model->Compute
<p align="center">
  <img src="docs/bn-dm.drawio.png" width="100%" />
</p>

GPTv4 as natural language interface to BN
<p align="center">
  <img src="docs/prompt1.png" width="30%" />
  <img src="docs/bn.png" width="30%" /> 
  <img src="docs/inference.png" width="30%" />
</p>

Applications of BN in decision making, PyAgrum code generated by GPT4V with iterative run-correct-run
```
...
for r in [0,1]:  # rainfall
    for rl in [0,1]:  # river_level
        for fb in [0,1]:  # flood_barriers
            for sb in [0,1]:  # sandbags
                if (r == 1 and rl == 1) or (fb == 0 and sb == 0):
                    bn.cpt(flood)[r, rl, fb, sb] = [0.1, 0.9]  # 10% No flood, 90% Flood
                else:
                    bn.cpt(flood)[r, rl, fb, sb] = [0.8, 0.2]  # 80% No flood, 20% Flood
...
for f in [0,1]:  # flood
    for es in [0,1]:  # emergency_services
        # High chance of drowning if there's a flood, especially with low-quality emergency services
        if f == 1 and es == 0:
            bn.cpt(people_drown)[f, es] = [0.2, 0.8]  # 20% No, 80% Yes
        elif f == 1 and es == 1:
            bn.cpt(people_drown)[f, es] = [0.5, 0.5]  # 50% No, 50% Yes
        else:
            bn.cpt(people_drown)[f, es] = [0.95, 0.05]  # 95% No, 5% Yes
...
for f in [0,1]:  # flood
    # Higher chance of houses being ruined if there's a flood
    if f == 1:
        bn.cpt(houses_ruined)[f] = [0.3, 0.7]  # 30% Not ruined, 70% Ruined
    else:
        bn.cpt(houses_ruined)[f] = [0.95, 0.05]  # 95% Not ruined, 5% Ruined
...
4. Diagnostics: Identifying the Causes of a Flood
...
```
5. Predictions: Assessing the Risk of People Drowning during a Flood
...
```
6. Predictions: Estimating the Risk of Houses Getting Ruined
...
7. Decision Making under Uncertainty (Decision Networks)
