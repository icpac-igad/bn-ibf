#!/usr/bin/env python3
"""
Drought Impact-Based Forecasting using Bayesian Networks - Version 5

Key improvements over v4:
1. GCS IceChunk/Zarr data integration for CDI and SPI3
2. Support for all East Africa seasons: MAM, JJA, SON, OND, JFM
3. Automatic lead time calculation for each season
4. Empirical probability computation from raw SPI3 ensemble data
5. Batch processing for multiple seasons/years
...
# Return period thresholds for empirical probability
RETURN_PERIOD_THRESHOLDS = {
    '3yr': -0.43,   # ~33% exceedance
    '5yr': -0.68,   # ~20% exceedance
    '10yr': -0.84,  # ~10% exceedance
    '20yr': -1.04,  # ~5% exceedance
    '50yr': -1.28,  # ~2% exceedance
}
...
class GCSCDILoader(GCSDataLoader):
    ...
    def get_cdi_path(self, year: int, month: int) -> str:
        ...
        # Pattern: bn_icpac_cdi_store/icechunk_10km/{year}/eadw-cdi-data-{year}-{month}
        # Note: No .zarr extension for IceChunk stores
...
class GCSSPI3Loader(GCSDataLoader):
    ...
    def get_spi3_for_lead(self, ds: xr.Dataset, lead: int) -> xr.DataArray:
        ...
        if 'lead' in spi3.dims:
            # Handle both 0-indexed and value-based selection
    ...
    def compute_empirical_probabilities(self, spi3: xr.DataArray,
                                         target_lat: np.ndarray = None,
                                         target_lon: np.ndarray = None,
                                         regridder: 'xe.Regridder' = None) -> Dict[str, xr.DataArray]:
        """
        Compute empirical exceedance probabilities from SPI3 ensemble.
        ...
        logger.info(f"Computing empirical probabilities from {n_members} ensemble members")
        logger.info(f"SPI3 original resolution: {spi3.lat.size} x {spi3.lon.size}")

        # Regrid SPI3 to target 10km grid if target coordinates provided
        if target_lat is not None and target_lon is not None:
            logger.info(f"Regridding SPI3 to target resolution: {len(target_lat)} x {len(target_lon)}")

            # Need to regrid each member separately, then recombine
            ...
            for member_idx in range(n_members):
                ...
                if HAS_XESMF and regridder is None:
                    # Build regridder on first pass
                    source_ds = member_data.to_dataset(name='spi3')

                    # Ensure ascending lat for xesmf
            ...
            # Stack back to member dimension
        ...
        eprobs = {}
        for rp_name, threshold in RETURN_PERIOD_THRESHOLDS.items():
            below = (spi3 <= threshold).sum(dim='member')
            prob = below / n_members
            prob.name = f'eprob_{rp_name}'
            eprobs[f'eprob_{rp_name}'] = prob
...
class DroughtDataLoaderV5:
    ...
    def __init__(self,
        ...
        self.spi3_loader = GCSSPI3Loader(service_account_file)
    ...
    def _extract_boundary_value_numpy(self, data_np: np.ndarray, boundary_mask: np.ndarray,
        ...
        if n_pixels > 0:
            # Use pre-computed numpy array directly (fast!)
            masked = data_np[boundary_mask]
            return float(np.nanmean(masked))
        else:
            # Fall back to xarray for centroid extraction (only when n_pixels=0)
    ...
    def prepare_analysis_data(self,
        ...
        3. Compute empirical probabilities at 10km
        ...
        logger.info("="*60)
        logger.info(f"PREPARING ANALYSIS DATA (V5 - GCS IceChunk)")
        ...
        # Handle different coordinate naming conventions (lat/lon vs x/y)
        if 'lat' in cdi_ref.coords:
            target_lat = cdi_ref.lat.values
            target_lon = cdi_ref.lon.values
        elif 'y' in cdi_ref.coords:
            # CDI uses x/y coordinates - these are the actual lat/lon values
        ...
        # Prepare CDI data (already at 10km, just extract variable and normalize coords)
        # OPTIMIZATION: Pre-compute numpy arrays to avoid lazy evaluation in boundary loop
        logger.info("Pre-computing CDI arrays to numpy (avoiding lazy evaluation)...")
        cdi_data_regridded = {}
        cdi_data_numpy = {}  # Store pre-computed numpy arrays
        for (year, month), cdi_ds in cdi_data_raw.items():
            ...
            # Pre-compute to numpy array ONCE
            cdi_data_numpy[(year, month)] = cdi_var.values

        # STEP 2: Load SPI3 forecast data (~100km)
        ...
        # Extract SPI3 for the specific lead time
        ...
        # STEP 3: Compute empirical probabilities with regridding to 10km
        logger.info(f"\nStep 3: Computing empirical probabilities (regridding to 10km)...")
        eprob_data = self.spi3_loader.compute_empirical_probabilities(
            spi3_lead,
            target_lat=target_lat,
            target_lon=target_lon
        )
        logger.info(f"Computed empirical probabilities for {len(eprob_data)} return periods at 10km")

        # OPTIMIZATION: Pre-compute eprob numpy arrays to avoid lazy evaluation in boundary loop
        logger.info("Pre-computing eprob arrays to numpy (avoiding lazy evaluation)...")
        eprob_data_numpy = {}
        ...
        for idx, row in self.boundaries.iterrows():
            ...
            # Extract multi-month CDI using pre-computed numpy arrays
            cdi_time_series = []
            ...
            # Compute weighted mean CDI using pre-computed numpy arrays
            ...
            # Extract empirical probabilities using pre-computed numpy arrays
            rp_probs = {}
            ...
            # Spatial coverage using pre-computed numpy array
            if n_pixels > 0:
...
class DroughtBayesianNetworkV5:
    """
    Bayesian Network for drought risk with GCS data integration.
    Same structure as V4, optimized for batch seasonal processing.
    """
    ...
    def _setup_cpds(self):
        """Setup Conditional Probability Distributions (same as V4)."""
        # Prior CPDs
    ...
    def _create_risk_cpt(self) -> TabularCPD:
        """Create risk level CPT (same logic as V4)."""
        if self.include_confidence_node:
    ...
    def _compute_risk_probs(self, antecedent: int, exceed: int, spatial: int,
        ...
        """Compute risk probabilities (same logic as V4)."""
...
def analyze_season(boundaries_path: str,
    ...
    """
    Run drought analysis for a specific season.
    ...
    logger.info("="*70)
    logger.info(f"DROUGHT BN ANALYSIS V5 - {season} Season")
    logger.info("="*70)

    # Initialize data loader
    loader = DroughtDataLoaderV5(
    ...
    # Initialize BN
    logger.info(f"\nInitializing Bayesian Network (V5)...")
    bn = DroughtBayesianNetworkV5(include_confidence_node=include_confidence_node)
    ...
    # Save results
    output_path = Path(output_dir) / f"drought_bn_v5_{season}_{init_year}_{init_month:02d}.csv"
...
def analyze_year_all_seasons(boundaries_path: str,
    ...
    """
    Run analysis for all seasons in a year.
    ...
    for season in seasons:
        ...
        # Get recommended init months for this season
        init_configs = get_recommended_init_months(season)

        # Use the first (longest lead) configuration
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Drought BN IBF V5 - GCS IceChunk")
    ...
    if args.year_all_seasons:
        ...
        if args.init_month is None:
            # Auto-calculate best init month for longest lead
