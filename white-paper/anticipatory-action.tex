...
\usepackage{authblk}
\title{Decision to Issue Drought Warning \\via Bayesian Network}
...
$S_\kappa=$ {\bf Severity } of drought is defined as minus SPI (higher rain,
higher SPI, less 
severe drought). 
For flood (FAA), we would
use $+SPI$ instead.
...
Let $X^i_\kappa\in \bool$ for $i=1,2, 3, \ldots, nx$ represent a flag for local problems
...
$IC_\kappa^i=$  insurance (i.e., preventive) cost for flag $X^i_\kappa$
...
$P_{danger, \kappa}^i=
\frac{IC_\kappa^i}{RC_\kappa^i}$ probability of danger for flag $X_\kappa^i$.
...
$W_\kappa=$  drought warning, ether  or 1
...
\beq
W_\kappa=
\indi\left[P(\rvS_\kappa\geq S_\kappa^*  ) > P^*_\kappa\right]
\eeq
...
=
\indi[
P(\rvS_\kappa>S^*_\kappa) >P^*_\kappa]
\eeq

\beq\color{blue}
P(Y_\kappa^i|W_\kappa)
...
evidence for
nodes $P(\rvS_\kappa> S^*_\kappa)$, $(X^1, X^2, X^3)$
...
\section{Relation to Climada}

This project used the Climada (see Ref.\cite{climada}) software library.
Table \ref{tab-climada} relates their 
terminology to ours.

In Climada, a hazard model 
...
Climada terminology & Our terminology \\ \hline
Intensity of event & $S$=severity of event  \\ \hline
Vulnerability=
$\frac{Impact}{Exposure}$ & $P_{danger}= \frac{IC}{RC}$=danger probability \\ \hline
Impact & $IC$= insurance cost (\$) \\ \hline
Exposure & $RC$=replacement cost (\$) \\ \hline
\end{tabular}
\caption{Relationship between Climada terminology and ours}
\label{tab-climada}
\end{table}

Climada takes as 
input the intensity of an
event, and calculates from this the event's
vulnerability, impact and exposure.
In this
project, we use Climada for each flag $X^i$.

\section{Risk}
\beqa
Risk_\kappa &=& E[\rv{IC}_\kappa] = \frac{1}{M} \sum_{\mu=1}^{M} IC^{\kappa,\mu}
\\
&=&\sum_{IC_\kappa} P(IC_\kappa) IC_\kappa
\eeqa

A {\bf risk matrix} is
a  $4\times 4$ matrix
with $X$ axis 
equal to insurance cost ($IC$) (a.k.a. impact)
and $Y$ axis equal to likelihood
$P$, with 
$IC$ binned into four values
and $P$ binned into 4 values.
The matrix elements give
the value of
some arbitrary function 
$f(IC, P)$ defined as risk. Usually, $f(IC, P)=IC*P$ but not necessarily. I find 
risk matrices arbitrary
and not very useful. The 
Wikipedia article on risk matrices (Ref.\cite{wiki-risk-matrix}) agrees.


\section{Damage Functions}

Bring the following disaster prediction models together via
Damage Function
...
$DF_\alp(x_\alp)$ is a function of the
...
is  tolerable. 
Any time total Damage is higher than this, always issue drought warning.

We assume $x_\alp$ is  binned and $x_\alp\geq 0$.
However, we will use $x_\alp={\tt None}$ to indicate
...
When $x_\alp\neq  {\tt None}$, it should be the
value of $x_\alp$ for some historical event.
...
previously, for drought kind $\kappa$. We 
will use this as a threshold for triggering  a  drought warning
